    ...
    while(1){
        ...
        if(n > 0){
            token = strtok(buf, " ");
						...
						if(strcmp(token,"exit") == 0){
							  ...
							  token = strtok(NULL, " ");  // get next token (value)
								...
								sprintf(sbuf,"%s%d", ACK, val); // Return ACK and the value
						}
						// --- START ADDED OPERATIONS ---
							  ...
							  token = strtok(NULL, " ");  // get next token (index)
								...
								sprintf(sbuf,"%s%d@%d", ACK, val, idx); // Return ACK, value, and index
						}
						else if(strcmp(token,"remove_back") == 0){
								// Make a Call
								val = list_remove_from_back(mylist);
								sprintf(sbuf,"%d", val); // Return the removed value
						}
							  ...
							  token = strtok(NULL, " ");
								...
								sprintf(sbuf,"%d", val); // Return the value at index
						}
						// --- END ADDED OPERATIONS ---
            ...
            send(clientSocket, sbuf, sizeof(sbuf), 0);
				...
				memset(buf, '\0', 1024);
				memset(sbuf, '\0', 1024); // Clear sbuf as well
    }
